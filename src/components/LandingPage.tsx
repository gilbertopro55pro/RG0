...
function FeatureBullets({ items }: { items: string[] }) {
  return (
    <ul className="space-y-1.5">
      {items.map((line) => (
        <li key={line} className="flex items-start gap-2 text-xs text-ink-soft leading-relaxed">
          <span className="mt-1 shrink-0 h-1.5 w-1.5 rounded-full" style={{ background: "var(--color-amber-deep)" }} />
          {line}
        </li>
      ))}
    </ul>
...
const GALLERY_BULLETS = [
  "קישור אישי ומאובטח לכל אירוע — בחירת תמונות, הורדה, ושיתוף גמיש (וואטסאפ, קוד QR ועוד)",
  "5 ערכות עיצוב מלאות (קלאסי, דרמטי, מינימלי, חם, רומנטי), כל אחת עם פריסה, צבעים וטיפוגרפיה משלה",
  "העלאת תמונות ישירות מהטלפון, כולל מאייפון",
  "זיהוי פרצופים אוטומטי (בדפדפן, בלי לשלוח תמונה לשום שרת חיצוני) — סינון הגלריה לפי מי מופיע/ה בתמונה בלחיצה אחת",
];

const ALBUM_TOOL_BULLETS = [
  "אשף בנייה אוטומטית: בוחרים מספר עמודים, כמות תמונות וסגנון — והמערכת בונה שבלונה מלאה לכל האלבום שנשאר רק לגרור תמונות לתוכה; אפשר לשמור מבנה מוצלח כתבנית לשימוש חוזר",
  "כלי proofing מובנה: הלקוח/ה עוברים על עמודי האלבום, כותבים הערות ומאשרים את העיצוב הסופי ישירות מהגלריה",
  "עיצוב חופשי עם כמה תמונות שרוצים בעמוד, או מספריית תבניות מוכנות (עשרות תבניות, ואפשר לשמור תבניות משלכם)",
  "מיקום וזום של כל תמונה בתוך המסגרת, עם מרכוז אוטומטי בלחיצה אחת ונקודות מיקוד לחיתוך",
  "קווי יישור חכמים שמסתנכרנים אוטומטית מול מרכז העמוד ותמונות סמוכות",
  "מסגרת הדפסה שמראה מראש את גבולות הבטיחות של העמוד",
  "אפקטים לכל תמונה — שחור-לבן, ספיה, שקיפות, טשטוש, סיבוב וצל — ומסגרות בעובי וצבע לבחירה",
  "הוספת כמה תמונות בבת אחת, עם סידור אוטומטי בעמוד לפי כיוון (אורך/רוחב)",
  "שכבות טקסט עם בחירה מ-20 פונטים (עברית ואנגלית, כולל כתבי יד), ותמונת רקע לעמוד",
  "קביעת גודל האלבום בס״מ וייצוא מוכן להדפסה: PDF, JPG באיכות דפוס, או קובץ פוטושופ (PSD) עם שכבות אמיתיות",
];
...
const FEATURES = [
  ...
  },
  {
    ...
    text: (
      <>
        <FeatureBullets items={GALLERY_BULLETS} />
        <div className="text-xs font-bold mt-3 mb-1.5" style={{ color: "var(--color-amber-deep)" }}>
          ובנוסף — כלי מובנה לעיצוב אלבומים:
        </div>
        <FeatureBullets items={ALBUM_TOOL_BULLETS} />
      </>
    ),
  },
  {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {FEATURES.map(({ icon: Icon, title, text }) => (
            <div key={title} className="rounded-2xl p-5 bg-card border border-line shadow-card">
              ...
              <div className="text-xs text-ink-soft leading-relaxed">{text}</div>
            </div>
          ))}
        </div>
      </section>
